...
Import-Module (Join-Path $ScriptRoot 'TerminALL.Common.psm1') -Force
...
$Messages = @{
    en = @{
        ...
        WslUnresponsive = 'WSL is not responding after quick retries. Try wsl --shutdown, then inspect wsl --status and wsl --list --verbose.'
        ...
        DistroUnresponsive = 'The distro {0} is not responding after quick retries. Try wsl --shutdown and inspect wsl --list --verbose.'
    ...
    es = @{
        ...
        WslUnresponsive = 'WSL no responde despues de intentos rapidos. Prueba wsl --shutdown y revisa wsl --status y wsl --list --verbose.'
        ...
        DistroUnresponsive = 'La distro {0} no responde despues de intentos rapidos. Prueba wsl --shutdown y revisa wsl --list --verbose.'
    ...
    try {
        ...
        if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
            ...
            return [pscustomobject]@{
                TimedOut = $true
                ExitCode = $null
                StdOut = ConvertTo-FlatText (Get-Content -Path $stdoutPath -ErrorAction SilentlyContinue)
                StdErr = ConvertTo-FlatText (Get-Content -Path $stderrPath -ErrorAction SilentlyContinue)
            }
        }
        ...
        if (-not $IgnoreExitCode -and $result.ExitCode -ne 0) {
            $details = ConvertTo-FlatText @($result.StdErr, $result.StdOut)
            if (-not $details) {
                $details = "wsl.exe exited with code $($result.ExitCode)."
            }

            throw (ConvertTo-FlatText $details)
        }
...
function Assert-WslResponsive {
    for ($attempt = 1; $attempt -le 3; $attempt++) {
        $result = Invoke-WslCommand -ArgumentList @('--status') -TimeoutSeconds 12 -IgnoreExitCode
        if (-not $result.TimedOut -and ($null -eq $result.ExitCode -or $result.ExitCode -eq 0)) { return }
...
function Show-WslRecommendations {
    ...
    Write-Host ' - First try: wsl --shutdown; then inspect wsl --status and wsl --list --verbose.'
    Write-Host ' - Primero prueba: wsl --shutdown; luego revisa wsl --status y wsl --list --verbose.'
    Write-Host ' - Update WSL manually with: wsl --update'
    Write-Host ' - Actualiza WSL manualmente con: wsl --update'
    Write-Host ' - Reboot Windows only if the previous diagnostics fail.'
    Write-Host ' - Reinicia Windows solo si fallan los diagnosticos anteriores.'
    Write-Host ''
}
...
function Set-LocalTerminalSelection {
    param([Parameter(Mandatory = $true)][string]$ExecutablePath)

    $resolved = [System.IO.Path]::GetFullPath($ExecutablePath)
    if (-not (Test-Path -LiteralPath $resolved -PathType Leaf)) {
        throw "WindowsTerminal.exe no existe: $resolved"
    }
    if ([System.IO.Path]::GetFileName($resolved) -ne 'WindowsTerminal.exe') {
        throw "El ejecutable configurado debe llamarse WindowsTerminal.exe: $resolved"
    }
    $configPath = Get-TerminALLLocalConfigPath
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $configPath) | Out-Null
    @{ terminalExecutable = $resolved } | ConvertTo-Json | Set-Content -LiteralPath $configPath -Encoding UTF8
    Write-Ok "Windows Terminal portable seleccionado: $resolved"
}

function Ensure-KaliWsl2 {
    $result = Invoke-WslCommand -ArgumentList @('-l', '-v') -TimeoutSeconds 20 -IgnoreExitCode
    if ($result.TimedOut -or $result.ExitCode -ne 0) {
        throw (T 'DistroUnresponsive' 'kali-linux')
    }

    $kaliLine = @((ConvertTo-FlatText $result.StdOut) -split "`r?`n" | Where-Object { $_ -match '(?i)kali-linux' }) | Select-Object -First 1
    if (-not $kaliLine) { throw "No se pudo comprobar la version WSL de kali-linux." }
    if ($kaliLine -match '\s2\s*$') { return }

    Write-Info 'Converting only kali-linux to WSL 2 / Convirtiendo solamente kali-linux a WSL 2...'
    & wsl.exe --set-version kali-linux 2 | Out-Host
    if ($LASTEXITCODE -ne 0) { throw 'No se pudo mantener kali-linux en WSL 2.' }
}
...
function Test-DistroReady {
    ...
    $normalizedOutput = ConvertTo-FlatText $result.StdOut
...
function Test-WindowsTerminalInstalled {
    try {
        [void](Resolve-TerminALLTerminalExecutable -RepoRoot $ScriptRoot)
        return $true
    }
    catch { return $false }
}
...
function Get-WindowsTerminalSettingsPath {
    return Resolve-TerminALLSettingsPath -RepoRoot $ScriptRoot -CreateStoreDirectory
}
...
function Set-TerminalActions {
    ...
    $zoomBinding = @{ keys = 'alt+shift+enter' }
    ...
    foreach ($action in $existing) {
        ...
        if ($keys -contains 'ctrl+left' -or $keys -contains 'ctrl+right' -or $keys -contains 'ctrl+up' -or $keys -contains 'ctrl+down' -or $keys -contains $zoomBinding.keys) {
            continue
        }
...
function Configure-WindowsTerminal {
    ...
    $kaliCommand = 'wsl.exe -d kali-linux -u root'
    $ubuntuCommand = 'wsl.exe -d Ubuntu'
    $powerShellCommand = 'powershell.exe'
    # Layout is passed explicitly by launch-terminal-layout.ps1. A global value
    # would affect unrelated Windows Terminal windows.
    Remove-ObjectProperty -Object $settings -Name 'startupActions'
    ...
    # Opaque colors keep Linux LS_COLORS readable on Windows 10 portable builds.
    Ensure-ObjectProperty -Object $defaults -Name 'useAcrylic' -Value $false
    Ensure-ObjectProperty -Object $defaults -Name 'opacity' -Value 100
    ...
    Ensure-ObjectProperty -Object $defaults -Name 'experimental.retroTerminalEffect' -Value $false
...
function Disable-ConflictingTerminalHotkeys {
    param([Parameter(Mandatory = $true)][string]$CurrentShortcutPath)

    $desktop = [Environment]::GetFolderPath('Desktop')
    $shell = New-Object -ComObject WScript.Shell
    foreach ($file in @(Get-ChildItem -LiteralPath $desktop -Filter '*.lnk' -File -ErrorAction SilentlyContinue)) {
        if ([string]::Equals($file.FullName, $CurrentShortcutPath, [StringComparison]::OrdinalIgnoreCase)) { continue }
        try {
            $shortcut = $shell.CreateShortcut($file.FullName)
            $normalized = ([string]$shortcut.Hotkey -replace '[+\s]', '').ToLowerInvariant()
            if ($normalized -notin @('ctrlaltt', 'altctrlt')) { continue }
            $backup = '{0}.{1}.bak' -f $file.FullName, (Get-Date -Format 'yyyyMMddHHmmss')
            Copy-Item -LiteralPath $file.FullName -Destination $backup -Force
            $shortcut.Hotkey = ''
            $shortcut.Save()
            Write-Info "Hotkey retirado de $($file.Name); respaldo: $backup"
        }
        catch {
            Write-Warn "No se pudo revisar el hotkey de $($file.FullName): $($_.Exception.Message)"
        }
    }
    $filtered += [pscustomobject]@{
        command = [pscustomobject]@{ action = 'togglePaneZoom' }
        keys = $zoomBinding.keys
    }
}
...
function Create-LauncherShortcuts {
    ...
    $selectedTerminal = Resolve-TerminALLTerminalExecutable -RepoRoot $ScriptRoot
    $terminalIcon = if (Test-Path $bundledIcon) {
        $bundledIcon
    }
    elseif (Test-Path -LiteralPath $selectedTerminal -PathType Leaf) {
        $selectedTerminal
    }
    else {
    ...
    if (Test-Path -LiteralPath $desktopShortcut) {
        $shortcutBackup = '{0}.{1}.bak' -f $desktopShortcut, (Get-Date -Format 'yyyyMMddHHmmss')
        Copy-Item -LiteralPath $desktopShortcut -Destination $shortcutBackup -Force
        Write-Info (T 'BackupCreated' $shortcutBackup)
    }
    Disable-ConflictingTerminalHotkeys -CurrentShortcutPath $desktopShortcut
...
try {
    ...
    if ($TerminalExecutable) {
        Set-LocalTerminalSelection -ExecutablePath $TerminalExecutable
    }
    $backgroundImage = if ($SkipBackground) { $null } else { Get-BackgroundImage }
    if (-not $ConfigureOnly) {
        ...
        Write-Info 'ConfigureOnly: se omiten cambios de WSL, distribuciones y herramientas Kali.'
    }
